...
class RegisterPatternsTest extends WP_UnitTestCase {
	...
	/**
	 * スロットル時間内でAPIが呼ばれないことをテスト
	 */
	public function test_vbp_register_patterns_throttles_api_calls_within_interval() {
		update_option(
			'vk_block_patterns_options',
			array(
				'VWSMail'           => 'throttle-test@example.com',
				'disableXT9Pattern' => false,
			)
		);

		// 最後のAPI呼び出し時刻を現在時刻に設定（スロットル時間内）.
		update_option( 'vk_patterns_api_last_call', time() );

		$api_call_count = 0;

		$http_filter = function() use ( &$api_call_count ) {
			$api_call_count++;
			return array(
				'body'     => wp_json_encode(
					array(
		...
		$result = vbp_register_patterns( null, 'lightning' );

		remove_filter( 'pre_http_request', $http_filter, 10 );
		delete_option( 'vk_block_patterns_options' );
		delete_option( 'vk_patterns_api_last_call' );

		// スロットル時間内なのでAPIが呼ばれないこと.
	...
	/**
	 * スロットル時間経過後にAPIが呼ばれることをテスト
	 */
	public function test_vbp_register_patterns_allows_api_calls_after_throttle_interval() {
		update_option(
			'vk_block_patterns_options',
			array(
				'VWSMail'           => 'throttle-test@example.com',
		...
		// 最後のAPI呼び出し時刻をスロットル時間より前に設定.
		update_option( 'vk_patterns_api_last_call', time() - VBP_API_THROTTLE_SECONDS - 1 );
		...
		// スロットル時間経過後なのでAPIが呼ばれること.
		$this->assertSame( 1, $api_call_count );
	}

	/**
	 * API呼び出し時刻が正しく記録されることをテスト
	 */
	public function test_vbp_register_patterns_records_api_call_time() {
		update_option(
			'vk_block_patterns_options',
			array(
				'VWSMail'           => 'throttle-test@example.com',
		...
		// 古いAPI呼び出し時刻を設定.
		$old_time = time() - VBP_API_THROTTLE_SECONDS - 100;
		update_option( 'vk_patterns_api_last_call', $old_time );
		...
		$recorded_time = (int) get_option( 'vk_patterns_api_last_call' );
		...
		// 記録された時刻が呼び出し前後の範囲内であること.
		$this->assertGreaterThanOrEqual( $before_call, $recorded_time );
		$this->assertLessThanOrEqual( $after_call, $recorded_time );
	}

	/**
	 * メールが未設定の場合はスロットルチェックが行われないことをテスト
	 */
	public function test_vbp_register_patterns_skips_throttle_when_no_email() {
		update_option(
			'vk_block_patterns_options',
			array(
				'VWSMail' => '',
